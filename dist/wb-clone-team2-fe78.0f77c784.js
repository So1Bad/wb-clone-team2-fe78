...
class productItem {
    constructor(name, price, discount1, finalPrice1, inCart1 = false, path){
        this.id = crypto.randomUUID().substr(2, 5);
        this.name = name;
        this.price = price;
        this.discount = discount1;
        this.finalPrice = finalPrice1;
        this.path = path;
        this.inCart = inCart1;
    }
    finalPrice() {
        this.finalPrice = this.price - this.price * discount;
    }
    isInCart() {
        const items = document.querySelector('.items');
        items.addEventListener('click', (event)=>{
            const quickVuiwBtn = event.target.closest('.card__bin');
            if (!quickVuiwBtn) return;
            const item = quickVuiwBtn.closest('.items__card');
            if (!item) return;
            quickVuiwBtn.classList.toggle('card__InCard');
            if (quickVuiwBtn.classList.contains('.card__InCard')) this.inCart = true;
        });
        return this.inCart;
    }
}
const card1 = new productItem("\u0428\u0442\u0430\u043D\u044B", 1000, 10, finalPrice, inCart = false);

},{}]},["7wZbQ","2R06K"], "2R06K", "parcelRequire5864", {})

//# sourceMappingURL=wb-clone-team2-fe78.0f77c784.js.map
