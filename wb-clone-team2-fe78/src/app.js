...
function initHeaderLogic() {
   ...
   cartBtn.addEventListener("click", () => {
      openCartModal(cardArr, toggleProductCartStatus, completeOrder);
   });
...
const toggleProductCartStatus = (productId) => {
...
const completeOrder = () => {
   cardArr.forEach((product) => {
      ...
      const cardElement = itemsContainer.querySelector(`.items__card[data-id="${product.id}"]`);
      if (cardElement) {
         const binBtn = cardElement.querySelector('.card__bin');
         if (binBtn) {
            binBtn.classList.remove('card__InCard');
         }
      }
   });
